...
from app.services.logger import summarize_user_theme_attempts
...
def generate_task(payload: TaskGenerationRequest) -> TaskGenerationResponse:
    ...
    try:
        llm_task, llm_detail = generate_llm_task_with_detail(
            interaction_type=payload.interaction_type, level_theme=payload.level_theme, difficulty=difficulty, language=payload.language,
            encounter_name=payload.encounter_name, encounter_style=payload.encounter_style, gameplay_context=payload.gameplay_context,
            structure_focus=payload.structure_focus, boss_mechanic=payload.boss_mechanic, tutorial_mode=payload.tutorial_mode,
            adaptation_reason=adaptation_reason,
        )
        if llm_task is not None:
            return llm_task
        return _fallback_task(payload, difficulty, adaptation_reason, llm_detail)
    except Exception as exc:
        return _fallback_task(payload, difficulty, adaptation_reason, "Unexpected backend fallback: %s" % exc.__class__.__name__)
