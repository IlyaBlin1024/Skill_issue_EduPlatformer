...
@onready var lesson_progress_label: Label = $Margin/VBox/LessonProgressLabel
...
func set_level_context(text: String) -> void:
	level_label.text = text
	level_label.visible = not text.is_empty()
...
func set_lesson_progress(text: String) -> void:
	lesson_progress_label.text = text
	lesson_progress_label.visible = not text.is_empty()
...
func set_terminal_overlay_mode(active: bool) -> void:
	$Margin.visible = not active
	if active:
		_message_version += 1
		message_label.text = ""
