...
func _update_level_objective() -> void:
	...
	if remaining_enemies > 0:
		hud.set_objective(_active_objective_text(level_id, room_role))
		return
...
func _active_objective_text(level_id: String, room_role: String) -> String:
	if level_id == "level_02":
		match room_role:
			"altar":
				return "Objective: Claim the forge, then clear the logic sentries."
			"chest":
				return "Objective: Secure the branch cache, then keep descending."
			"exit":
				return "Objective: Decision gate is sealed. Resolve the sentries first."
			_:
				return "Objective: Resolve every logic sentry in Level 2."
	if level_id == "level_03":
		match room_role:
			"altar":
				return "Objective: Reach the loop forge, then break the wave sentries."
			"chest":
				return "Objective: Secure the cycle cache, then continue through the conduit."
			"exit":
				return "Objective: Conduit gate is sealed. Collapse the wave sentries first."
			_:
				return "Objective: Break every loop sentry in Level 3."
	if level_id == "level_04":
		match room_role:
			"altar":
				return "Objective: Claim the function forge, then clear the archivist sentries."
			"chest":
				return "Objective: Secure the helper cache, then continue the archive climb."
			"exit":
				return "Objective: Function gate is sealed. Defeat the archivist sentries first."
			_:
				return "Objective: Defeat every function sentry in Level 4."
	if level_id == "level_05":
		match room_role:
			"altar":
				return "Objective: Claim the integration forge, then stabilize the system sentries."
			"chest":
				return "Objective: Secure the system cache, then complete the final route."
			"exit":
				return "Objective: System gate is sealed. Stabilize every sentry first."
			_:
				return "Objective: Stabilize every system sentry in Level 5."
	match room_role:
		"altar":
...
func _cleared_objective_text(level_id: String, room_role: String) -> String:
	if level_id == "level_02":
		match room_role:
			...
			_:
				return "Objective: Path is stable. Head to the decision gate."
	if level_id == "level_03":
		match room_role:
			"exit":
				return "Objective: Reach the conduit gate and leave the loop foundry."
			_:
				return "Objective: The loop path is clear. Head to the conduit gate."
	if level_id == "level_04":
		match room_role:
			"exit":
				return "Objective: Reach the function gate and leave the codex archive."
			_:
				return "Objective: The function path is clear. Head to the function gate."
	if level_id == "level_05":
		match room_role:
			"exit":
				return "Objective: Reach the system gate and complete the final exam."
			_:
				return "Objective: The system path is clear. Head to the final gate."
	match room_role:
		"exit":
...
func _apply_player_combo_step(target: EnemyEncounter, combo_mode: String, combo_index: int, facing_direction: Vector2) -> void:
	...
	if combo_mode == "ranged":
		var ranged_damage: int = PLAYER_RANGED_COMBO_DAMAGE[min(combo_index, PLAYER_RANGED_COMBO_DAMAGE.size() - 1)]
	...
	if not _can_player_melee_target(target):
		...
		return
	var melee_damage: int = PLAYER_MELEE_COMBO_DAMAGE[min(combo_index, PLAYER_MELEE_COMBO_DAMAGE.size() - 1)]
